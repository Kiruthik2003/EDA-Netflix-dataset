...
x = df['country'].value_counts().head(5).index
y1 = df[df['type'] == 'Movie']['country'].value_counts().reindex(x, fill_value=0)
y2 = df[df['type'] == 'TV Show']['country'].value_counts().reindex(x, fill_value=0)
plt.bar(x, y1, color='#a60059', label='Movies')
plt.bar(x, y2, bottom=y1, color='#33ccff', label='TV Shows')
plt.xlabel('Country')
plt.ylabel('Count')
